...
HF_DATASETS_CACHE="./huggingface_cache" USE_MODELSCOPE_HUB=1 DISABLE_VERSION_CHECK=1  USE_MCA=1 torchrun --nnodes=$GEMINI_TASKS_NUM --node_rank=$GEMINI_TASK_INDEX --master_addr=$GEMINI_HOST_IP_taskrole1_0 --master_port=29500 --nproc_per_node=8 src/train.py \
    ...
    --do_train \
    --train_from_scratch \
    --model_name_or_path DeepseekV32-6B-mca-baseline-moe-bias-update-ratio-5000 \
    --dataset fineweb_edu_350b \
    --finetuning_type full \
    --output_dir saves/DeepseekV32-6B-mca-baseline-moe-bias-update-ratio-5000/pt/fineweb_edu_350b \
    --overwrite_cache \
    ...
    --report_to wandb \
    --run_name DeepseekV32-6B-mca-baseline-moe-bias-update-ratio-5000_pt_fineweb_edu_350b
