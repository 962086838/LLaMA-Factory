...
class TopKRouter(Router):
    ...
    def apply_input_jitter(self, input: torch.Tensor):
        ...
        """
        if self.config.moe_input_jitter_eps is not None and self.config.moe_input_jitter_eps > 0:
            # print(f"apply_input_jitter: {self.config.moe_input_jitter_eps}")
    ...
    def routing(self, logits: torch.Tensor):
        ...
        try:
            ...
            if (not dist.is_initialized() or dist.get_rank() == 0) and getattr(self, "layer_number", 0) == 1:
                print(f"[Megatron-Core Router Layer {self.layer_number}] moe_router_bias_update_rate = {self.config.moe_router_bias_update_rate}", flush=True)
        except:
            pass
        # =========================
