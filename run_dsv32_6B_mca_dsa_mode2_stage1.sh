...
sleep 3
# Load non-DSA weights, train indexer only (using --freeze_non_indexer True)
HF_DATASETS_CACHE="./huggingface_cache" USE_MODELSCOPE_HUB=1 DISABLE_VERSION_CHECK=1  USE_MCA=1 torchrun --nnodes=$GEMINI_TASKS_NUM --node_rank=$GEMINI_TASK_INDEX --master_addr=$GEMINI_HOST_IP_taskrole1_0 --master_port=29500 --nproc_per_node=8 src/train.py \
    ...
    --output_dir saves/DeepseekV32-6B-mca-dsa-mode2-stage1/pt/fineweb_edu_350b \
    --overwrite_cache \
    ...
    --freeze_non_indexer True \
    --report_to wandb \
    --run_name DeepseekV32-6B-mca-dsa-mode2-stage1_pt_fineweb_edu_350b
